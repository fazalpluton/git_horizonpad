...
describe("Horizon pad",  function ()  {
  ...
  it("Should stake zpad", async function () {
    ...
    let approve = await zpad.connect(per1).approve(staking.address,_value)
    await approve.wait()
    let tx = await staking.connect(per1).stake(_value)
    await tx.wait()
    ...
    await tx.wait()
    // approve = await zpad.connect(per2).approve(staking.address,_value)
    // await approve.wait()
    //  tx = await staking.connect(per2).stake(_value)
    // await tx.wait()
    await mineNBlocks(1);
  });
  it("states", async function () {
    ...
    tx = await staking.getUserStakedValue(per1.address)
    console.log("staked value :",tx.toString())
